...
public class ChenShuTemplate {
	...
	private static List<Tree> getIP(Tree t) {
		...
		if (!t.child.isEmpty()) {
			if (t.content.equals("IP") && t.child.size() > 1 && checkIP(t) && 
					(!Print.print(t).contains("比") || !Print.print(t).contains("比例")))
				l.add(t);
			else
				if (t.child.size() != 0)
	...
	public static String getTemplate(Tree w, ArrayList<String> sentence) throws IOException {
		String template = "";
		List<Tree> l = getIP(w);
		/*Set<String> set1 = new HashSet<String>();
		for (int i = 1; i < 11; i++)
			if (i != 2)
				set1.addAll(Input.getWord(i));*/
		Set<String> set = new HashSet<String>();
		/*for (String st : set1)
			if (st.length() > 1) set.add(st);
		set.remove("形成");
		set.add("最");
		set.add("比");*/
		set.add("推断");
		...
		for (Tree t : l) {
			...
			for (int index = 0; index < tl.size(); index++) {
				...
				if (tvp.child.size() >= 2 && (tvp.child.get(tvp.child.size() - 2).content.startsWith("V") || tvp.child.get(tvp.child.size() - 2).content.startsWith("P"))) {
					...
					else if (temp.child.size() == 0 && t.child.size() > 2 && t.child.get(t.child.indexOf(tnp) - 1).content.startsWith("N"))
						s = Print.print(t.child.get(t.child.indexOf(tnp) - 1)) + "@" + Print.print(temp) + middle + Print.print(tvp)+ "\t";
					else {
						s = Print.print(temp.child.get(0));
						if (s.endsWith("的"))
